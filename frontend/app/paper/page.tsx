...
import { planEsatPaperSlots, planTmuaPaperSlots, startBackgroundGeneration, slotCounts } from '@/lib/queue';
import type { TmuaPaperSelection } from '@/lib/queue';
...
const MODULE_COLORS: Record<string, { bg: string; text: string; ring: string; dot: string; bar: string }> = {
  ...
  'TMUA Paper 1':        { bg: 'bg-blue-50 dark:bg-blue-950/30',    text: 'text-blue-700 dark:text-blue-300',   ring: 'ring-blue-300 dark:ring-blue-700',   dot: 'bg-blue-400',   bar: 'bg-blue-400' },
  'TMUA Paper 2':        { bg: 'bg-indigo-50 dark:bg-indigo-950/30', text: 'text-indigo-700 dark:text-indigo-300', ring: 'ring-indigo-300 dark:ring-indigo-700', dot: 'bg-indigo-400', bar: 'bg-indigo-400' },
};
...
export default function PaperModePage() {
  ...
  const [tmuaPaperSel, setTmuaPaperSel] = useState<TmuaPaperSelection>('full');
  ...
  function handleStart() {
    ...
    const isTmua = examType === 'TMUA';
    const mods = isTmua
      ? (tmuaPaperSel === 'full' ? ['TMUA Paper 1', 'TMUA Paper 2']
          : tmuaPaperSel === 'paper2' ? ['TMUA Paper 2']
          : ['TMUA Paper 1'])
      : ['Mathematics', ...selectedOptional];
    const allSlots = isTmua
      ? planTmuaPaperSlots(tmuaPaperSel, bankFraction, difficulty, excludedYears)
      : planEsatPaperSlots(mods, bankFraction, difficulty, excludedYears);
    const sessionId = uuidv4();
    const now = Date.now();
    // TMUA real exam: 75 min per paper
    const tmuaExamMins = tmuaPaperSel === 'full' ? 150 : 75;
    const timerSec = examMode
      ? (isTmua ? tmuaExamMins : 120) * 60
      : customTimerEnabled
  ...
  const canStart = examType === 'TMUA' || selectedOptional.length === 2;
  const allModules = examType === 'TMUA'
    ? (tmuaPaperSel === 'full' ? ['TMUA Paper 1', 'TMUA Paper 2']
        : tmuaPaperSel === 'paper2' ? ['TMUA Paper 2']
        : ['TMUA Paper 1'])
    : ['Mathematics', ...selectedOptional];
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 pb-12">
        <div className="max-w-xl mx-auto">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mt-6 mb-1">Paper Mode</h1>
          <p className="text-sm text-gray-500 mb-3">
            {examType === 'ESAT'
              ? '60 questions across 3 modules · Mathematics is mandatory'
              : 'Past paper questions · Mathematical thinking & applications'}
          </p>
          ...
          <section className="mb-5">
            {examType === 'ESAT' ? (
              ...
              <>
                <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Select paper</h2>
                <div className="space-y-2">
                  {([
                    { value: 'paper1', label: 'Paper 1', desc: 'Mathematical thinking & proof · 20 questions · 75 min' },
                    { value: 'paper2', label: 'Paper 2', desc: 'Applications & statistics · 20 questions · 75 min' },
                    { value: 'full',   label: 'Full Sitting', desc: 'Both papers · 40 questions · 150 min' },
                  ] as const).map(({ value, label, desc }) => (
                    <button
                      key={value}
                      onClick={() => setTmuaPaperSel(value)}
                      className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-colors text-left ${
                        tmuaPaperSel === value
                          ? 'bg-blue-50 dark:bg-blue-950/20 border-blue-400 dark:border-blue-600 text-blue-800 dark:text-blue-300'
                          : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600'
                      }`}
                    >
                      <div
                        className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                          tmuaPaperSel === value ? 'border-blue-500 bg-blue-500' : 'border-gray-300 dark:border-gray-600'
                        }`}
                      >
                        {tmuaPaperSel === value && <span className="w-2 h-2 rounded-full bg-white block" />}
                      </div>
                      <div>
          ...
          {canStart && (
            <div className="mb-4 p-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-600 dark:text-gray-400">
              ...
              <div className="text-xs text-gray-400">
                {allModules.length * 20} questions ·{' '}
                {bankPct > 0 && aiPct > 0
                ...
                {' '}· {examMode
                  ? examType === 'TMUA'
                    ? `${tmuaPaperSel === 'full' ? '150' : '75'}min exam mode`
                    : '2h exam mode'
                  : customTimerEnabled ? `${customTimerMinutes}m timer` : 'no timer'}
